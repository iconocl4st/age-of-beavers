    ...
    private final class ThresholdSelector extends JPanel {
        ...
        void drawDemand(EntityReader entity, boolean recreate) {
            synchronized (sync) {
                ...
                if (recreate) {
                    int max = Math.min(currentPrioritization.maximumAmount, capacity.totalWeight / resourceType.weight);
                    maximum.setModel(new DefaultBoundedRangeModel(currentPrioritization.desiredAmount, 1, 0, max + 1));
