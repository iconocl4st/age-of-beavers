...
public class SelectionListener implements MouseListener, MouseMotionListener {
    ...
    public SelectionListener(ClientContext context, RectangleListener listener, Zoom zoom, JPanel panel) {
    ...
    public void mousePressed(MouseEvent mouseEvent) {
        ...
        Set<EntityId> displayables = context.gameState.locationManager.getEntities(
                destination,
                entity -> !context.gameState.hiddenManager.get(entity)
        );

        if (displayables.isEmpty()) {
            context.selectionManager.select(displayables);
            return;
        }
        ...
        EntitySpec type = context.gameState.typeManager.get(displayables.iterator().next());
