...
public class Zoom {
    ...
    public Zoom(JPanel panel, ClientContext context) {
        this.panel = panel;
    ...
    public void focusOn(Set<EntityId> entityIds) {
        double x = 0;
        double y = 0;
        int count = 0;
        for (EntityId entity : entityIds) {
            DPoint location = context.gameState.locationManager.getLocation(entity);
